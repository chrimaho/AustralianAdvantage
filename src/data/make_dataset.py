...
# Fix the data frame ----
def set_FixData(DataFrame, raw, name="POA"):
    """
    ...
    # Convert data
    data.iloc[:,1] = data.iloc[:,1].replace(utils.get_DataLabels(raw, name, "id"))
    ...
    data.iloc[:,5] = data.iloc[:,5].replace(utils.get_DataLabels(raw, name, "name"))
...
def main():
    ...
    for dataset in config.DataSets:
        ...
        # Get data
        raw = utils.get_RawData(source)
        utils.let_DumpData(raw, os.path.join(project_dir, "data/raw"), "Seifa2016_"+name+"_Raw.json")
        
        # Process data
        data = utils.set_DataFrame(raw)
        utils.let_DumpData(data, os.path.join(project_dir, "data/raw"), "Seifa2016_"+name+"_Raw.csv")
        data = set_FixData(data, raw)
        utils.let_DumpData(data, os.path.join(project_dir, "data/processed"), TargetFileName="Seifa2016_"+name+"_Processed.csv")
        # print(data)
